...
Игры представляют собой набор ROMов от NES со срезанными копирайтами. Видимо, чтобы маркетплейсы не отклоняли устройства из-за незаконного распространения пиратского контента)) Сам набор игр достаточно странный. Из интересного есть только классический тетрис, остальные какие-то не особо интересные ноунейм игры.
...
Видим разъем для micro-SD карты (1), кнопку спуска затвора (2), кнопочку сброса устройства (3), аж две контактные площади для батарейки (4), 24-пиновый коннектор камеры (5), дублирующиеся контактные площадки для динамика (6), небольшой микрофон (7), 22-пиновый MIPI коннектор для LCD (8) и кварц (9).  Из интересного на шелкографии можно заметить серийный номер и дату изготовления платы. Забавно, что камера была куплена недавно, а дата производства платы 2020. Видимо, когда-то было напечатано огромное количество таких плат, что их до сих пор продают. Поиск серийника в интернете, очевидно, ни к чему не привел.
...
Эта функция представляет собой реализацию простого Bootloader. Он ответственен за базовую инициализацию перифирии, поиск основного приложения/ОС во flash, обновление прошивки и проч.

Проанализировав функции, расположенные рядом с bootloader-ом, нашел довольно много интересного, например, ту самую закладку с обновлением устройства через SD-карту, которой мы уже смогли воспользоваться. 
...
![image](files/main_ui_decompressed.png)    
...
![image](files/pic_1.jpg) 
![image](files/pic_2.jpg) 
![image](files/pic_3.jpg) 
...
Во-вторых, решил добавить немного розовых и черных страз на корпус камеры, вдохновившись картинкой из пинтереста. 

В итоге получилось примерно так: 

![alt text](files/P1011218.JPG)
![alt text](files/P1011220.JPG)
